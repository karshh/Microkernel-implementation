...
void trainServer(){
	...
	//run initialization
	initTrains(csTID,commandServerTID, dspTID, trainSpeed);
	...
	while(1){
		...
		bwassert(msgLen >= 0, COM2, "<trainServer>: Receive error.\r\n");
			...
			//right now dummy vars
			//will shunt to velocity server later
		if (_tid == sensorTID) {
		//it is up to sensor Server/ velocity servers to keep track of train information
/*
...
*/
		//}else if( _tid == velTID){
			//train has stopped, set speed to 0
			//not train server's problem anymore velTd keeps track of it now
			//trainSpeed[58] = 0;
			//Reply(_tid, "1", 2);
		} else {
