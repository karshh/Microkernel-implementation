...
int getEdgeDistance(TrackGraph * t, int s1, int s2) {
    ...
    switch(node[s1].type) {
        case Sensor:
            ...
            break;
        case Switch:
            if (node[s1].switchConfig == C && node[s1].CnextNodeIndex == s2) return node[s1].CnextNodeDistance;
            if (node[s1].switchConfig == S && node[s1].SnextNodeIndex == s2) return node[s1].SnextNodeDistance;
            break;
        case MultiSwitch:
            if (node[s1].switchConfig == CS && node[s1].CSnextNodeIndex == s2) return node[s1].CSnextNodeIndex;
            if (node[s1].switchConfig == SC && node[s1].SCnextNodeIndex == s2) return node[s1].SCnextNodeIndex;
            break;
        default:
            bwassert(0, COM2, "<findEdge>: couldn't find edge between %d and %d", s1, s2);
            break;
